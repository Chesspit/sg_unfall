...
table = dbc.Card(
    dbc.CardBody(
        [
            html.H4("Dash Table / AG Grid", className="card-title"),
            html.P(
                "Some quick example text to build on the card title and make "
                "up the bulk of the card's content.",
                className="card-text",
...
# Tabelle für Auswertung nach Häufigkeit
...
tabelle = dash_table.DataTable(
    df_weekday_hour.to_dict('records'),
    [{"name": i, "id": i} for i in df_weekday_hour.columns]
)


cl_all = dbc.Card(
    [
                dbc.Label("Typ des Unfalls"),
                dbc.Checklist(
                            id="checklist_typ",
                            options=[
                                {"label": x, "value": x}
                ...
                dbc.Label("Beteiligte am Unfall"), 
...
slider_card_start = dbc.Card(
    [
        html.H5("Start Datum", className="card-title"),
        dcc.Slider(
            id="start_monat",
            marks={i: f"{i}" for i in range(1, 13, 1)},
            min=1,
            max=12,
            step=1,
            value=5,
...
slider_card_ende = dbc.Card(
    [
        html.H5("Ende Datum", className="card-title"),
        html.Div("(Monat)", className="card-title"),
        dcc.Slider(
            id="ende_monat",
            marks={i: f"{i}" for i in range(1, 13, 1)},
            min=1,
        ...
        html.Div("(Jahr)", className="card-title"),
        dcc.Slider(
            id="ende_jahr",
            marks={i: f"{i}" for i in range(2011, 2023, 1)},
            min=2011,
        ...
        dbc.Row(
            [
                ...
                dbc.Col([html.P("Häufigkeit nach Wochentag und Stunde"),
                    tabelle], width=12, lg=8, className="mt-4 border"),
            ],
            className="ms-1",
        ),
                dbc.Row(
...
def update_map(checklist_typ, checklist_schwere, checklist_beteiligte, checklist_strasse, start_monat, start_jahr, ende_monat, ende_jahr):
    ...
    # filter_query = '''
    #                 @start <= Datum <= @ende 
    #                 and Unfalltyp in @checklist_typ 
    #                 and Unfallschwere in @checklist_schwere 
    #                 # and mit Fussgänger in @checklist_beteiligte 
    #                 and Strassentyp in @checklist_strasse
    #             '''
    # df_filtered = df.eval(filter_query)
